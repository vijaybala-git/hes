...
def _apply_share_blob(blob: str) -> bool:
    ...
    if not clean:
        return False
    apply_config(clean)   # records the loaded rate context; the CAGR seeder won't clobber it
    return True
...
def _SettingsLoadDialog(open_rv, err_rv):
    ...
    def _apply(source):
        try:
            apply_config(config.load_config(source))   # loaded CAGR reproduces verbatim
...
def Page():
    ...
    model, df = solara.use_memo(run_simulation, dependencies=[
        ...
        solar_battery_rte_pct.value, solar_battery_charge_kw.value,
        solar_battery_discharge_kw.value,
        solar_battery_grid_charging.value, elec_tariff_label.value,
        ...
        wh_setpoint_f.value, hpwh_ambient_location.value,
