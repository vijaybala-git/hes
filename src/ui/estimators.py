...
from devices.physics import AMBIENT_COP_FACTOR
...
def _est_hpwh(uef: float, daily_gal: int,
              avg_inlet_f: float = 60.0, setpoint_f: float = 120.0,
              ambient_location: str = "conditioned") -> float:
    factor = AMBIENT_COP_FACTOR.get(ambient_location, 1.0)
    return daily_gal * 365 * 8.33 * (setpoint_f - avg_inlet_f) * 0.000293 / (uef * factor)
