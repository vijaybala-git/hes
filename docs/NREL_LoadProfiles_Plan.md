...
| Access | Anonymous S3 (`us-west-2`); column-subset parquet reads ≈ 0.6 s per building (only the needed columns cross the wire). Needs `pyarrow` (**build-time only** — in `requirements-build.txt`, never imported by `src/` nor installed in the deployed app). |
